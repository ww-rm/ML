...
class Perceptron:
    ...
    def fit(self, train_x, train_y, iter_times=10000000, yita=0.01):
        """
        参数: 
            x: 二维数组类型, 每一行是一个样本
            y: 类型格式与x相同, 值取+1或-1
        ...
        # 获得特征数与样本数
        self.total_num = len(train_y)
        self.feature_num = len(train_x[0])
        ...
        for _ in range(iter_times):
            if train_y[index] * (np.matmul(self.w, train_x[index]) + self.b) <= 0:
                self.w = np.add(self.w, np.multiply(yita*train_y[index], train_x[index]))
    ...
    def predict(self, x):
        return np.sign(np.matmul(self.w, x) + self.b)
